    ...
    [](ConversionCtx* ctx, const torch::jit::Node* n, args& args) -> bool {
     ...
     [](ConversionCtx* ctx, const torch::jit::Node* n, args& args) -> bool {
        ...
        if(args[3].IValue()->isNone() && args[4].IValue()->isNone())
        {
         ...
         fc.nbFields = f.size();
         fc.fields = f.data();
         nvinfer1::IPluginV2* pluginV2 = ctx->mPluginRegistry.at(pluginName)->createPlugin("instancenorm1", &fc);
         ...
         instance_norm_layer->setName("instancenorm1");
        ...
        R"SIG(aten::group_norm_old(Tensor input, int? num_groups, Tensor? weight,
            ...
            if (ctx->input_is_dynamic) {
                ...
                beta = args[3].unwrapToTensor(at::full({shape}, 1, {options}));
            ...
            if(true)
            {
                ...
                fc.nbFields = f.size();
                fc.fields = f.data();
                nvinfer1::IPluginV2* pluginV2 = ctx->mPluginRegistry.at(pluginName)->createPlugin(util::node_info(n).c_str(), &fc);
                ...
                auto layer = ctx->net->addPluginV2(inputs.data(), int(inputs.size()), *pluginV2);
            ...
            else
            {
                ...
                auto new_shape = util::toVec(input->getDimensions());
                int64_t c = new_shape[1];
                new_shape[1] = num_groups;
                new_shape[2] = c / num_groups * new_shape[2];
                LOG_DEBUG("New shape: " << new_shape);

                at::Tensor gamma_ = repeat_if_defined(gamma, num_groups);
                at::Tensor beta_ = repeat_if_defined(beta, num_groups);
                at::Tensor mean_ = repeat_if_defined(mean, num_groups);
                at::Tensor var_ = repeat_if_defined(var, num_groups);
                ...
                auto power = Weights(ctx, at::ones_like(scale));
                auto gn = ctx->net->addScaleNd(
                        *input, nvinfer1::ScaleMode::kCHANNEL, bias_weights.data, scale_weights.data, power.data, 1);
                gn->setName(util::node_info(n).c_str());
                auto out_tensor = gn->getOutput(0);
