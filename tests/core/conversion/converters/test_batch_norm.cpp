...
//        %5 : bool = prim::Constant[value=0]()
//        %6 : Tensor = aten::group_norm(%0, %3, %1, %2, %4, %5)
//        return (%6))IR";
//
//  auto g = std::make_shared<torch::jit::Graph>();
//  torch::jit::parseIR(graph, &*g);
//
////  auto in = at::randint(1, 10, {1, 6, 5, 5}, {at::kCUDA});
////  auto gamma = at::randint(1, 10, {6}, {at::kCUDA});
////  auto beta = at::randint(1, 10, {6}, {at::kCUDA});
//
//  auto in = at::ones({1, 6, 5, 5}, {at::kCUDA});
//  auto gamma = at::ones({6}, {at::kCUDA});
//  auto beta = at::zeros({6}, {at::kCUDA});
//
//  auto trt_in = at::clone(in);
//  auto trt_gamma = at::clone(gamma);
//  auto trt_beta = at::clone(beta);
...
//
//
//  params = trtorch::core::conversion::get_named_params(g->inputs(), {trt_gamma, trt_beta});
//  auto trt_results = trtorch::tests::util::RunGraphEngine(g, params, {trt_in});
//
//  ASSERT_TRUE(trtorch::tests::util::almostEqual(jit_results[0], trt_results[0].reshape_as(jit_results[0]), 5e-6));
//}